    ...
    <Card className="border-border/50 shadow-lg">
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between">
          ...
          <span className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-medium border ${getStatusColor(todayRecord?.status)}`}>
      ...
      <CardContent className="space-y-4">
        {todayRecord ? (
          <>
            ...
            <div className="flex items-start gap-4 p-4 rounded-xl bg-success/5 border border-success/10">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-lg font-bold text-success">
                  {format(todayRecord.checkInTime!, 'h:mm a')}
                </p>
            ...
            {/* Check Out Info */}
            <div className={`flex items-start gap-4 p-4 rounded-xl ${todayRecord.checkOutTime ? 'bg-warning/5 border border-warning/10' : 'bg-muted/50 border border-border/50'}`}>
              <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-full ${todayRecord.checkOutTime ? 'bg-warning/10' : 'bg-muted'}`}>
                ...
                {todayRecord.checkOutTime ? (
                  <>
                    <p className="text-lg font-bold text-warning">
                      {format(todayRecord.checkOutTime, 'h:mm a')}
                    </p>
